...
def convert_neigh_list_to_edge_index(nn_idx: Tensor, is_batched: bool=True) -> Tensor:
    ...
    with torch.no_grad():
        if is_batched:
            ...
            idx_offset = (torch.arange(B).to(nn_idx.device) * N).reshape(B, 1, 1)
            ...
            edge_index = torch.stack([center_idx, nn_idx], dim=1).to(nn_idx.device) # B, 2, N, K
            edge_index = edge_index.flatten(2) # B, 2, |E| per batch
            edge_index = edge_index.transpose(0, 1).transpose(1, 2) # 2, |E| per batch, B
            edge_index = edge_index.flatten(1).contiguous() # 2, |E|
        else:
            center_idx = torch.arange(N).to(nn_idx.device).unsqueeze(-1).repeat(1, K)
            edge_index = torch.stack([center_idx, nn_idx], dim=0).to(nn_idx.device) # 2, N, K
            edge_index = edge_index.flatten(1).contiguous() # 2, |E|
